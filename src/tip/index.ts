...
function isValidNonNegativeFinite(value: number): boolean {
 ...
 */
export function calculateTip(monto: number, porcentaje: number): TipResult {
  if (!isValidNonNegativeFinite(monto)) {
    throw new Error(
      `monto inválido: se esperaba un número finito no negativo, se recibió ${monto}`
    );
  }

  if (!isValidNonNegativeFinite(porcentaje)) {
    throw new Error(
      `porcentaje inválido: se esperaba un número finito no negativo, se recibió ${porcentaje}`
    );
  }
